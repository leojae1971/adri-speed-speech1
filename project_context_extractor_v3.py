        ...
        for node in ast.walk(self.ast_tree):
            if isinstance(node, ast.ClassDef):
                ...
                if isinstance(node, ast.FunctionDef):
                    ...
                    # También contar async como función (opcional)
                    info['functions'].append(func)

                # Detectar endpoints HTTP en decoradores
                for decorator in node.decorator_list:
                    if isinstance(decorator, ast.Call):
                        if isinstance(decorator.func, ast.Attribute):
                            method_name = decorator.func.attr.lower()
                            if method_name in HTTP_METHODS:
                                info['endpoints'].append({'method':method_name.upper(), 'line':node.lineno})
                        elif isinstance(decorator.func, ast.Name):
                            if decorator.func.id.lower() in HTTP_METHODS:
                                info['endpoints'].append({'method':decorator.func.id.upper(), 'line':node.lineno})
                    elif isinstance(decorator, ast.Name):
                        if decorator.id.lower() in HTTP_METHODS:
                            info['endpoints'].append({'method':decorator.id.upper(), 'line':node.lineno})
